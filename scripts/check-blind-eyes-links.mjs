...
const directory = JSON.parse(await readFile(new URL("../research/blind-eyes/directory.json", import.meta.url), "utf8"));
const urls = new Set([directory.scripture.url]);

for (const source of directory.definition_sources) urls.add(source.url);

for (const profile of directory.profiles) {
  for (const item of profile.evidence) urls.add(item.url);
  for (const teaching of profile.teachings) urls.add(teaching.url);
}
...
async function check(url) {
  try {
    const response = await fetch(url, {
      redirect: "follow",
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; TDS-Evidence-Archive/1.0; blind-eyes-link-audit)",
        Range: "bytes=0-1023",
      },
...
console.log(`Checked ${results.length} Blind Eyes links: ${results.length - failed.length - restricted.length} reachable, ${restricted.length} access-restricted, ${failed.length} failed.`);
