...
test("applies the four additional tests to every Evidence eBox without forcing a conclusion", () => {
  assert.ok((html.match(/class="case-test-note case-test-note--patriotic"/g) ?? []).length === 82, "expected a Patriotic Test lens on every case");
  assert.ok((html.match(/class="case-test-note case-test-note--america-first"/g) ?? []).length === 82, "expected an America First Test lens on every case");
  assert.ok((html.match(/class="case-test-note case-test-note--deal"/g) ?? []).length === 82, "expected a Deal Test lens on every case");
  assert.ok((html.match(/class="case-test-note case-test-note--world-standing"/g) ?? []).length === 82, "expected a World Standing Test lens on every case");
  ...
  assert.ok((html.match(/class="test-score-badge test-score-badge--patriotic"/g) ?? []).length === 82, "expected a Patriotic score on every case");
  assert.ok((html.match(/class="test-score-badge test-score-badge--america-first"/g) ?? []).length === 82, "expected an America First score on every case");
  assert.ok((html.match(/class="test-score-badge test-score-badge--deal"/g) ?? []).length === 82, "expected a Deal score on every case");
  assert.ok((html.match(/class="test-score-badge test-score-badge--world-standing"/g) ?? []).length === 82, "expected a World Standing score on every case");
...
test("adds a share composer trigger to every evidence eBox", () => {
  assert.ok((html.match(/class="ebox-share-trigger"/g) ?? []).length === 82, "expected every archive case file to be shareable");
  ...
  assert.ok((html.match(/Share evidence<\/button>/g) ?? []).length === 82, "expected the archive share buttons to say Share evidence");
