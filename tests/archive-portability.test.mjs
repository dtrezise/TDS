...
test("normalizes the legacy research into a deterministic portable archive", async () => {
  ...
  assert.equal(first.counts.cases, 81);
  assert.equal(first.counts.case_aliases, 85);
  assert.equal(first.counts.claims, 170);
  assert.equal(first.counts.sources, 214);
  assert.equal(first.counts.claim_sources, 462);
  ...
  assert.equal(first.tables.claims.filter((claim) => claim.claim_type === "editorial_analysis").length, 64);
  assert.equal(first.tables.claims.filter((claim) => claim.claim_type === "faith_analysis").length, 21);
  assert.equal(first.tables.claim_sources.filter((citation) => citation.relationship === "supports").length, 231);
  assert.equal(first.tables.claim_sources.filter((citation) => citation.relationship === "contextualizes").length, 231);
